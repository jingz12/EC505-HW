%% change mean from 0 to 4
%meanvector=0:0.5:4;
noisevector=0:0.2:2;
pd=zeros(size(Gamma,2),size(noisevector,2));%pd=zeros(size(Gamma,2),size(meanvector,2));
pf=zeros(size(Gamma,2),size(noisevector,2));%pf=zeros(size(Gamma,2),size(meanvector,2));
mean=2;
for jj=1:size(noisevector,2)%size(meanvector,2)
    noise=noisevector(jj);%mean=meanvector(jj);
y0=randn(5000,1);
...
for ii=1:size(noisevector,2)%ii=1:size(meanvector,2)
...
xlabel('pf'),ylabel('pd'),title(['all det-','ROC-','ProblemA-noise'])
%plot(pf(10,5),pd(10,5),'r*')
plot(pf(10,6),pd(10,6),'r*')
print(gcf,'-dpng',['all det-','ROC-','ProblemA-noise'])
